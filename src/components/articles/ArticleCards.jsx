...
function ArticleCardsItem({ itemWrapper }) {
  ...
  const dateBadgeClass = largeTexts ? `text-2` : `text-2`;

  return (
    <div className={`article-cards-item`}>
      {itemWrapper.link && itemWrapper.link.href && (
        <Link
          href={itemWrapper.link.href}
          className={`article-cards-item-link`}
        >
          <CircularButton
            faIcon={
              itemWrapper.link.faIcon || `fa-solid fa-arrow-up-right-dots`
            }
      ...
      <div className={`article-cards-item-content`}>
        ...
        <div
          className={`article-cards-item-content-description ${textClass} mt-1`}
          dangerouslySetInnerHTML={{ __html: itemWrapper.locales.text }}
        />

        {itemWrapper.dateStart && (
          <DateBadge
            dateEnd={itemWrapper.dateStartDisplay}
