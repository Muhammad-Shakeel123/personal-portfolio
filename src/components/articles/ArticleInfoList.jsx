...
function ArticleInfoListItems({ dataWrapper, selectedItemCategoryId }) {
  ...
  const shrinkClass = filteredItems.find(
    itemWrapper => itemWrapper.locales.text,
  )
    ? ``
  ...
  useEffect(() => {
    const itemDivs = document
      .getElementById(id)
      .querySelectorAll(`.article-info-list-item`);
  ...
  return (
    <div className={`article-info-list-items ${shrinkClass}`} id={id}>
      {filteredItems.map((itemWrapper, key) => (
        <ArticleInfoListItem itemWrapper={itemWrapper} key={key} />
      ))}
    </div>
  );
}
...
function ArticleInfoListItem({ itemWrapper }) {
  ...
  const hoverClass = linkHovered ? `article-info-list-item-hovered` : ``;
  ...
  return (
    <div className={`article-info-list-item ${hoverClass}`}>
      ...
      <div className={`article-info-list-item-content`}>
        <div
          className={`article-info-list-item-info-title ${titleClass}`}
        ...
        {itemWrapper.link?.href && (
          <Link
            href={itemWrapper.link?.href || null}
