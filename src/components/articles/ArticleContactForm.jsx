 ...
 * @constructor
 */
function ArticleContactFormContent({
  dataWrapper,
    ...
    switch (status) {
      ...
      case ArticleContactForm.Status.SUBMITTED:
        utils.storage.setWindowVariable(
          id,
          ArticleContactForm.Status.SUBMITTED,
        );
  ...
  const _onSubmit = async e => {
    ...
    const apiValidation = api.validators.validateEmailRequest(
      name,
      email,
      subject,
      message,
    ...
    const fakeEmailRequests =
      utils.storage.getWindowVariable('fakeEmailRequests') || false;
  ...
  return (
    <RowForm id={id} onSubmit={_onSubmit}>
      ...
      {!didSubmit && (
        <ArticleContactFormContentFields
...
function ArticleContactFormSuccessMessage({ dataWrapper, email, onReset }) {
  ...
  return (
    <MessageCard>
      ...
      <MessageCardFooter
        text={dataWrapper.locales.contactThankYouFooter.replace(
          '$email',
          formattedEmail,
        )}
      >
