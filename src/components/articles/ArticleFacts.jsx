...
function ArticleFactsItems({ dataWrapper, selectedItemCategoryId }) {
  ...
  return (
    <Swipeable
      className={`article-facts-items`}
      slidesPerView={4}
      loop={true}
      breakpoints={constants.SWIPER_BREAKPOINTS_FOR_FOUR_SLIDES}
    >
      {filteredItems.map((itemWrapper, key) => (
        <ArticleFactsItem itemWrapper={itemWrapper} key={key} />
      ))}
    </Swipeable>
  );
}
  ...
  return (
    <div className={`article-facts-item`}>
      ...
      <div className={`article-facts-item-info`}>
        <h6
          className={`article-facts-item-info-title lead`}
        ...
        <div
          className={`article-facts-item-info-description text-3 mt-1`}
