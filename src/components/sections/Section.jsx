    ...
    <>
      ...
      {!isHidden && (
        <>
          ...
          <SectionFocusManager section={section} status={status} />
        </>
      )}
    </>
  );
...
function SectionRenderer({
  ...
  return (
    <section
      className={`section ${statusClassName}`}
...
function SectionTransitionManager({
  section,
  visible,
  shouldTransition,
  status,
  ...
  const _tweenOut = () => {
    ...
    scheduler.schedule(
      () => {
        setStatus(Section.Status.HIDING);
      },
      50,
      schedulerTag,
    );
  ...
  const _tweenIn = () => {
    ...
    scheduler.schedule(
      () => {
        setStatus(Section.Status.SHOWING);
      },
      250,
      schedulerTag,
    );
...
function SectionFocusManager({ section, status }) {
  ...
  useEffect(() => {
    if (status !== Section.Status.SHOWN) return;

    const keyId = input.lastKeyPressed?.id;
    if (keyId === 'ArrowUp' || keyId === 'ArrowDown') _focus(true);
  ...
  const _focus = () => {
    ...
    if (!sectionScrollableEl) return;

    if (!isFocusingForbidden) sectionScrollableEl.focus();
  };
  ...
  const _blur = () => {
    ...
    if (!isFocusingForbidden) sectionScrollableEl.blur();
  };
}
