...
function GalleryModal({ target, onDismiss }) {
  ...
  useEffect(() => {
    ...
    scheduler.interval(
      () => {
        const isReady =
          utils.dom.didLoadImagesWithQuerySelector('.swiper-image');
        if (!isReady) return;
  ...
  const visibilityClassName = didLoadAllImages ? `visible` : `invisible`;
    ...
    >
      ...
      <ModalWrapperBody className={`gallery-modal-body`}>
        ...
        {parameters.direction === 'vertical' && (
          <GalleryModalImageStack
            className={visibilityClassName}
            images={images}
          />
        )}

        {!didLoadAllImages && <GalleryModalSpinner />}
      </ModalWrapperBody>
    </ModalWrapper>
  );
}
  ...
  return (
    ...
    >
      {images.map((image, key) => (
        <SwiperSlide key={key} className={`gallery-swiper-slide`}>
          <img
...
function GalleryModalImageStack({ className, images }) {
  ...
  return (
    <div className={`gallery-modal-image-stack ${className}`}>
      {images.map((image, key) => (
        <div key={key} className={`gallery-modal-image-stack-item`}>
          <img
            className={`swiper-image`}
            alt={`img-` + key}
            src={utils.file.resolvePath(image)}
