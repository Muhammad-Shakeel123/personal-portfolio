...
function OptionPickerButton({
  ...
  const availableOptions = options.filter(
    option => showSelectedOptionOnDropdown || option.id !== selectedOption.id,
  );

  const buttonBehaviorEnabled =
    mode === OptionPickerButton.Modes.MODE_BUTTON ||
    (mode === OptionPickerButton.Modes.MODE_AUTO && options.length <= 2);

  const caretIcon =
    !buttonBehaviorEnabled && selectedOption.img
      ? `fa-solid fa-caret-down`
      : null;

  const _onToggleClicked = () => {
    if (!buttonBehaviorEnabled) return;

    const optionIndex = options.indexOf(selectedOption);
    const targetIndex = optionIndex < options.length - 1 ? optionIndex + 1 : 0;
...
function OptionPickerButtonMenu({
  ...
  const hasSelectedOption = availableOptions.some(
  ...
  return (
    <Dropdown.Menu>
      {availableOptions.map((option, key) => (
        <Dropdown.Item
          key={key}
          className={`btn-option-picker-menu-item ${borderClass} ${
            option.id === selectedOptionId
              ? 'btn-option-picker-menu-item-selected'
              : ''
          }`}
          onClick={() => {
            onClick(option);
...
function OptionPickerButtonPickerIcon({ option, size }) {
  ...
  return (
    <div className={`btn-option-picker-icon ${sizeClass}`}>
      {willRenderImage && (
        <img src={option.img} alt={option.label} className={`img`} />
      )}

      {willRenderFaIcon && <i className={`fa-icon ${option.faIcon}`} />}
    </div>
  );
}
