...
function AudioButton({
  ...
  const isMagicCursorEnabledAndActive =
    feedbacks.animatedCursorEnabled && feedbacks.animatedCursorActive;
  ...
  const _loadAudio = () => {
    ...
    audioRef.current.addEventListener('error', () => {
      ...
      utils.log.warn(
        'AudioButton',
        "Couldn't load audio from URL: " + resolvedPath,
      );
    });
  };
  ...
  return (
    <div className={`audio-button-wrapper ${size}`}>
      ...
      <button
        className={`audio-button ${buttonClassName}`}
        id={uniqueId}
