...
function RowForm({ children, id, className = '', onSubmit }) {
  ...
  return (
    <form id={id} onSubmit={onSubmit}>
      <Row className={`row-form gx-3 ${className}`}>{children}</Row>
    </form>
  );
}

function RowFormGroup({ children, className = '' }) {
  return <div className={`row-form-group ${className}`}>{children}</div>;
}

function RowFormGroupItem({ children, className = '' }) {
  return <div className={`row-form-group-item ${className}`}>{children}</div>;
}

function RowFormGroupAlert({ variant, message }) {
  return (
    <RowFormGroup className={`row-form-alert col-12`}>
      <Alert className={`text-3`} dismissible={false} variant={variant}>
