...
function LayoutAnimatedBackground() {
  ...
  const visibilityClass =
    utils.device.isAndroid() && !utils.device.isChromeAndroid() ? `d-none` : ``;
  ...
  const _draw = updatedCircles => {
    ...
    const circleColorLight = utils.css.getRootSCSSVariable(
      '--theme-background-contrast',
    );
    const circleColorDark = utils.css.getRootSCSSVariable(
    ...
    updatedCircles
      ...
      .forEach(circle => {
        ...
        context.fillStyle = utils.css.hexToRgba(
          circleColorDark,
          circle.opacity / 2,
        );
        context.fill();
  ...
  return (
    <Animable
      className={`layout-animated-background ${visibilityClass}`}
