...
function LayoutNavigation({
  ...
  const currentCategorySectionLinks = sectionLinks.filter(
    link => link.categoryId === targetCategoryId,
  );

  const shouldAddFooterOffset =
    viewport.getLayoutConstraints()?.shouldAddFooterOffset;
  const offsetClass = shouldAddFooterOffset
    ? `layout-navigation-children-wrapper-with-offset`
    : ``;

  return (
    <div className={`layout-navigation-wrapper`}>
      {!isMobileLayout && <NavSidebar profile={profile} links={sectionLinks} />}
