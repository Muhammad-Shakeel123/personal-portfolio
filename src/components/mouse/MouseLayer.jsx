...
function MouseLayer({ active, isBlockedByOverlay, hidden }) {
  ...
  useEffect(() => {
    ...
    scheduler.schedule(
      ...
      100,
      'overlay-changed-animated-cursor',
    );
  }, [isBlockedByOverlay]);
  ...
  const _updateMouseTargetParameters = () => {
    ...
    if (input.isClicked && targetElementParameters?.faIcon) return;

    const parameters = _generateMouseTargetParameters();
    if (parameters) setTargetElementParameters(parameters);
    else setTargetElementParameters(null);
  };

  const _generateMouseTargetParameters = () => {
    if (!input.lastMouseTarget) return;

    let parameters = null;
    if (input.lastMouseTarget.matches('a')) parameters = { type: 'link' };
