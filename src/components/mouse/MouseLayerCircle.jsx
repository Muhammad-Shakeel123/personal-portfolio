...
function MouseLayerCircle({
  ...
  const isPositioned =
    currentX === input.mouseMoveStatus.clientX &&
    currentY === input.mouseMoveStatus.clientY;

  const shouldFadeOut =
    stoppedFor > MAX_IDLE_TIME_IN_SECONDS && !targetElementParameters;
  ...
  const _updateTransform = circleDiv => {
    ...
    if (input.isClicked) targetScale = MAX_SCALE;
    if (hidden) targetScale = 0;
    ...
    circleDiv.style.transform = `translate3d(${
      currentX - CIRCLE_SIZE_IN_PIXELS / 2
    }px, ${currentY - CIRCLE_SIZE_IN_PIXELS / 2}px, 0) scale(${
      currentScale / 3
    })`;
  };
  ...
  const _updateOpacity = circleDiv => {
    let targetOpacity = 0;
    if (!shouldFadeOut) targetOpacity = MIN_OPACITY;
    if (targetElementParameters)
      targetOpacity = (MAX_OPACITY - MIN_OPACITY) / 2;
    if (input.isClicked) targetOpacity += (MAX_OPACITY - MIN_OPACITY) / 2;
    if (hidden) targetOpacity = 0;
  ...
  const _updateStoppedFor = () => {
    if (isPositioned) setStoppedFor(prevState => prevState + dt / 60);
    else setStoppedFor(0);
  };
...
function MouseLayerCircleIcon({ faIcon }) {
  const hiddenClass = faIcon ? `` : `mouse-layer-circle-fa-icon-hidden`;

  const displayClass = faIcon || `fa-solid fa-circle`;

  return (
    <i
      className={`mouse-layer-circle-fa-icon ${hiddenClass} ${displayClass}`}
    />
  );
}
