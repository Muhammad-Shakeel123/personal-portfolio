...
function NavLinkPillsFixed({ links, className = '' }) {
  ...
  useEffect(() => {
    ...
    if (navigation.nextSection || navigation.scheduledNextSection)
      setStatus(NavLinkPillsFixed.Status.HIDING);
    else if (navToolsEndY < 0 && navToolsStartY < 0)
      setStatus(NavLinkPillsFixed.Status.SHOWN);
    ...
    navigation.scheduledNextSection,
  ]);

  return (
    <div
      className={`nav-link-pills-fixed-wrapper nav-link-pills-fixed-wrapper-${status}`}
    >
      <NavLinkPills
        id={'nav-link-pills-fixed'}
        className={className}
        links={links}
      />
    </div>
