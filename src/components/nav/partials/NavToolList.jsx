...
function NavToolList({ expanded }) {
  ...
  const widgets = [
    ...
    ...(feedbacks.animatedCursorEnabled
      ? [NavToolSettings.Options.CURSOR]
      : []),
    ...(profile.resumePdfUrl ? [NavToolSettings.Options.DOWNLOAD_RESUME] : []),
  ];

  const visibleWidgets =
    widgets.length <= maxWidgets ? widgets : widgets.slice(0, maxWidgets - 1);

  const groupedWidgets =
    widgets.length <= maxWidgets ? [] : widgets.slice(maxWidgets - 1);
