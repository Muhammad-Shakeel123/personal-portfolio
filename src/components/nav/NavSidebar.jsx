...
function NavSidebar({ profile, links }) {
  ...
  const shrinkClass = expanded ? `` : `nav-sidebar-shrink`;

  useEffect(() => {
    if (shouldForceShrink) return;
  ...
  return (
    <nav
      className={`nav-sidebar ${constants.HTML_CLASSES.scrollbarDecorator} ${shrinkClass}`}
    >
      <Card className={`nav-sidebar-card-wrapper`}>
        ...
        <NavProfileCard profile={profile} expanded={expanded} />

        <NavLinkList links={links} expanded={expanded} />
