...
function TextTyper({
  ...
  useEffect(() => {
    ...
    setParsedStrings(
      strings.map(string => {
        return utils.string.stripHTMLTags(string);
      }),
    );
  }, [strings]);
  ...
  const _onStatusTyping = ticks => {
    setCursorVisible(false);
    if (statusElapsed <= typingSpeed) return;
  ...
  const _onStatusDeleting = () => {
    if (statusElapsed <= deletingSpeed) return;
...
function TextTyperSpan({ currentText, cursorVisible, status }) {
  const visibleClass = cursorVisible ? `text-typer-span-cursor-visible` : ``;

  const transitionClass =
    currentText.length === 0 ? `text-typer-span-cursor-no-transition` : ``;

  const mainText =
    status === TextTyper.Status.SHOWING
      ? currentText
      : currentText.slice(0, -1);

  const lastChar =
    status === TextTyper.Status.SHOWING ? `` : currentText.slice(-1);
  ...
  return (
    <span className={`text-typer-span`}>
      ...
      <span
        className={`text-typer-span-cursor ${visibleClass} ${transitionClass}`}
      >
        _
      </span>
    </span>
