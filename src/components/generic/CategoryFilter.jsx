...
function CategoryFilter({
  ...
  let minButtonWidthPercentage =
    categories && categories.length ? Math.floor(100 / categories.length) : 0;

  const hasCategoryWithLongLabel = categories.find(
    category => category.label && category.label.length >= 8,
  );
  if (hasCategoryWithLongLabel) minButtonWidthPercentage *= 0.9;
  ...
  return (
    <div className={`category-filter btn-group ${className}`} role={'group'}>
...
function CategoryFilterButton({
  ...
  const selectedClassName = isSelected ? `category-filter-button-selected` : ``;

  const tooltip = language
    .getString('filter_by')
    .replace('{x}', category.label);
