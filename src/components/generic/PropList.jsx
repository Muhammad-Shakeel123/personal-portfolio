...
function PropList({ children, className = ``, inlineBreakpoint = `xl` }) {
  ...
  const inlineClass = isInline ? `prop-list-inline` : ``;

  return (
    <ul className={`prop-list ${inlineClass} ${className}`}>{children}</ul>
  );
}
    ...
    <li className={`prop-list-item ${className}`}>
      ...
      {type === PropListItem.Types.INTERVAL && (
        <>
          ...
          <i
            className={`fa-solid fa-arrow-right-long mx-1 mx-sm-2 text-1 prop-list-separator`}
          />
      ...
      {type === PropListItem.Types.DUO && (
        <>
          <span dangerouslySetInnerHTML={{ __html: value[0] }} />
          <span className={`mx-1 px-1 px-sm-0 mx-sm-2 prop-list-separator`}>
            ·
          </span>
          <span dangerouslySetInnerHTML={{ __html: value[1] }} />
        </>
