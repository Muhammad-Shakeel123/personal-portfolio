...
function StatusCircle({
  variant,
  message,
  size = 'status-circle-size-default',
  className = '',
  onClick = null,
}) {
  const utils = useUtils();
  const [uniqueId, setUniqueId] = useState(
    utils.string.generateUniqueRandomString('status-circle-'),
  );

  return (
    <div
      className={`status-circle ${className} ${size} status-circle-variant-${variant}`}
      id={uniqueId}
    >
      <HoverStaticTooltip
        label={message}
