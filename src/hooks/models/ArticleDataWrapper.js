...
export default class ArticleDataWrapper {
  ...
  _parseSettings(rawData) {
    ...
    return {
      ...
      maxRowsCollapseThreshold:
        rawSettings['max_rows_collapse_threshold'] || undefined,
  ...
  _parseItems(rawData, language, theme) {
    ...
    return rawItems.map((rawItem, key) => {
      return new ArticleItemDataWrapper(
        this,
        rawItem,
        language,
        theme,
  ...
  _parseCategories(rawData, language) {
    ...
    if (
      !categorizeBy ||
      !Array.isArray(categorizeBy) ||
      categorizeBy.length === 0
    )
      return [];
    ...
    const categories = [
      {
        ...
        key: 0,
        all: true,
        label: language.getTranslation(
          rawData.locales,
          ArticleDataWrapper.CATEGORY_ALL,
        ),
