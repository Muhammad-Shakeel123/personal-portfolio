...
export default class ArticleItemDataWrapper {
  ...
  constructor(articleDataWrapper, rawData, language, theme, id) {
    ...
    this.dateStartDisplayAsExperienceTime = language.getExperienceTimeString(
      this.dateStart,
    );
  ...
  _parsePreview(rawPreview, language) {
    ...
    if (
      ...
      supportedRatios.indexOf(screenshotsAspectRatio) === -1
    ) {
    ...
    return {
      ...
      hasScreenshotsOrYoutubeVideo:
        screenshots.length > 0 || rawPreview.youtubeVideo,
  ...
  get imageAlt() {
    ...
    if (this.locales.title)
      return utils.string.stripHTMLTags(this.locales.title);
    return 'item-' + this.id;
  }
  ...
  _parsePropForListing(name, value) {
    ...
    if (value instanceof Date)
      ...
      value = utils.json.sanitizeForLogs(value);
