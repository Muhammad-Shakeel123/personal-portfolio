...
function InputProvider({ children }) {
  ...
  const [mouseDownStatus, setMouseDownStatus] = useState({
    clientX: null,
    clientY: null,
    count: -1,
  });
  const [mouseMoveStatus, setMouseMoveStatus] = useState({
    clientX: null,
    clientY: null,
    count: -1,
  });
  const [mouseUpStatus, setMouseUpStatus] = useState({
    clientX: null,
    clientY: null,
    count: -1,
  });
  ...
  const _onMouseUp = e => {
    ...
    scheduler.schedule(
      () => {
        const shouldReset = !utils.dom.isInsideElement(
          e.target,
          e.clientX,
          e.clientY,
        );
