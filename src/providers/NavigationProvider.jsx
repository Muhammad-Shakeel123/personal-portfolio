...
function NavigationProvider({ children, sections, categories }) {
  ...
  const [shouldForceScrollToTopCount, setShouldForceScrollToTopCount] =
    useState(false);
  ...
  const _adjustScrollBeforeTransition = () => {
    ...
    const didChangeCategory =
      targetSection?.category.id !== nextSection?.category.id;
  ...
  useEffect(() => {
    const isRunning =
      transitionStatus === NavigationProvider.TransitionStatus.RUNNING;
    const isFinishing =
      transitionStatus === NavigationProvider.TransitionStatus.FINISHING;
