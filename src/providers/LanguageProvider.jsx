...
function LanguageProvider({
  ...
  const defaultLanguage =
    allLanguages.find(lang => lang.id === defaultLanguageId) || allLanguages[0];
  ...
  useEffect(() => {
    if (allLanguages.length === 0) {
      utils.log.throwError(
        'LanguageProvider',
        'The app must support at least one language. Make sure you filled the supportedLanguages property in the settings.json file.',
      );
      return;
    }
  ...
  const getAvailableLanguages = excludeSelected => {
    ...
    if (!excludeSelected) return allLanguages;
  ...
  const getTranslation = (locales, key, customFallback) => {
    ...
    const selectedLanguageTranslation = _translate(
      locales[selectedLanguageId],
      key,
    );
    if (selectedLanguageTranslation) return selectedLanguageTranslation;

    const defaultLanguageTranslation = _translate(
  ...
  const parseJsonText = text => {
    ...
    return text
      .replace(
        /\{\{(.*?)\}\}/g,
        `<span class="${constants.HTML_CLASSES.textHighlight}">$1</span>`,
      )
  ...
  const getString = key => {
    const locales =
      appStrings && appStrings['locales'] ? appStrings['locales'] : {};
  ...
  const getDateLocaleString = date => {
    ...
    if (utils.date.isSameDay(date))
      return `<strong>${getString('present')}</strong>`;

    const localeString = date.toLocaleString(selectedLanguageId, {
      year: 'numeric',
      month: 'short',
    });
  ...
  const getExperienceTimeString = date => {
    ...
    if (floorYearsDiff > 1)
      return getString('experience_year_count_plural').replace(
        '{x}',
        floorYearsDiff + '+',
      );
    else if (floorYearsDiff === 1)
      return getString('experience_year_count_singular').replace(
        '{x}',
        floorYearsDiff.toString(),
      );
    else return getString('experience_year_count_less_than_one');
  };
